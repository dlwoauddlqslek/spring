...
create table product(
	pno int auto_increment,	-- 제품번호(권장)
	pName varchar(50),		-- 제품명
    pDesc text,				-- 제품설명
    pPrice int,				-- 제품가격
    pdate datetime default now(), -- 등록일(권장)
...
create table productimg(
	pimgno int auto_increment, -- 제품이미지 번호
    pimgname text,			   -- 제품이미지 이름
    pimgdate datetime default now(),
    pno int,
    foreign key(pno) references product(pno)
    on update cascade on delete cascade, -- pk삭제시 참조하는 fk 레코드도 같이 삭제
    primary key(pimgno)
