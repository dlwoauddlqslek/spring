...
# 1. 집계 함수, 미리 제공되는 함수들
select * from buy; # buy 테이블 모든 레코드 조회
select bamount+10 from buy; # 조회할 때 수량 필드의 연산 가능

# 1-1 sum(필드명) : 해당 필드의 총합계
select sum(bamount) from buy; # 수량의 총합계

# 1-2 avg(필드명) : 해당 필드의 평균
select avg(bamount) from buy; # 수량 필드의 평균

# 1-3 max(필드명) : 해당 필드의 최대값
select max(bamount) from buy; # 수량 필드의 최대값

# 1-4 min(필드명) : 해당 필드의 최소값
select min(bamount) from buy; # 수량 필드의 최소값

# 1-5 count(필드명) : 해당 필드의 (null제외)레코드 수
select count(bgname) from buy; # 분류 필드의 (null제외한) 레코드 수
# 1-5 count(*) : 해당 필드의 (null포함)레코드 수
select count(*) from buy;      # (null포함)레코드 수

# 2. 그룹, 조회 할 때 특정 필드 기준으로 그룹/묶음
select * from buy;
select * from buy group by mid;
#Error Code: 1055. Expression #1 of SELECT list is not in GROUP BY clause and contains nonaggregated column 'sqltest2.buy.bnum' which is not functionally dependent on columns in GROUP BY clause; this is incompatible with sql_mode=only_full_group_by
...
select mid,bamount from buy group by mid;
# RDBMS : 관계형 데이터베이스, 행과 열로 이루어진 테이블, 최소 단위 데이터=레코드(행) 단위

# - 2개 이상의 필드를 집계해서 조회할때는 그룹이 필수. 레코드 단위라서.
# - 총 집계 구하기 : 그룹 없이 가능, ??별(그룹)집계 : 그룹 필수
select sum(bamount) from buy;					# 수량 필드의 총 합계
select mid,sum(bamount) from buy;				# error
select mid,sum(bamount) from buy group by mid;	# 정상 : mid 그룹으로 수량 필드의 총합계
...
# 다양한 문법절 적용
select b.mid,sum(bamount*bprice) as total_price # [필드/연산식] as 별칭
from buy b inner join member m 					# [테이블] 별칭
